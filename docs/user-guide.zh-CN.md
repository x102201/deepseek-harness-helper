...
**一实例 = 一份独立 `DSH_HOME` = 一套固定插件。** 定向任务拆到不同实例，就不会在同一个工作空间里互相踩踏。插件安装与 Agent 预设仍在 **dsh 网页界面** 中完成；DSHHelper 负责实例的创建、运行、分屏与制品分发。
...
每个实例可以看成一个**专职数字员工**（客服 / 开发 / 测试）。要几个人一起磨同一份交付物：在**同一台电脑**里并排坐，打开**同一个项目目录**。工具箱仍隔离，文件共享；你在分屏里调度。拆到多台电脑上等于一人一间房，既浪费机器，也看不见全场。谁跟谁对接由你交代——员工并排坐着，转个头就行。DSHHelper 不改官网 dsh，只把工位摆到同一张桌子上。

![三个数字员工：并排坐 vs 一人一间房](assets/zh-CN/collab-model.svg)
...
`.dshpack` 卖的是「已跑通的实例」，不是插件清单。导入时本产品按规格在当前平台复原运行环境（Node / dsh 版本、补丁、预设、设置与声明的插件）。一份制品对应一个实例；调好几个角色，就分别导出几个包。导入不会复原分屏布局。
