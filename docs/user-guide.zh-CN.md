...
| 名词 | 含义 |
|------|------|
| **实例** | 一份独立的 DeepSeek Harness 工作间：自有进程、端口与数据目录（`DSH_HOME`），互不干扰 |
...
2. 若 Gatekeeper 拦截：在 Finder 中右键应用 → **打开**，或在「系统设置 → 隐私与安全性」中允许
...
`.dshpack` 卖的是「已跑通的实例」，不是插件清单。导入时本产品按规格在当前平台复原运行环境（Node / dsh 版本、补丁、预设、设置与声明的插件）。
...
| macOS「无法验证开发者」 | 右键 → 打开，或在系统设置中允许；见 [安装 · macOS](#macos) |
