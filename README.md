...
## 二、协作：三个专职数字员工，坐同一张桌子

**每一个 dsh 就是一个专职数字员工**：客服只干接待，开发只改代码，测试只盯清单。插件是他们的工具箱，所以必须分开——这是第一节。

但这三个人要交的是同一份活。把他们拆到三台电脑上，等于三个办公室各雇一个，机器多占三倍，文件还得传来传去，你也盯不全场。**数字员工要的是工位并排，不是一人一间房。**

做法是一台电脑里并排坐：**三个实例共用同一个项目目录**；工具箱分开，文件共享。你在分屏里当主管——左边改完一段，右边立刻对着同一份文件改说明，下面按清单验收。

这不是三个互不相干的窗口并排开，是一张桌子上三个数字员工磨**同一版**。

谁跟谁对接，你转个头交代一声就行。人就坐在旁边，这件事本来就不难。DSHHelper 不改官网 dsh——数字员工还是原厂那套，Helper 只负责把工位摆到同一张桌子上。

![三个数字员工：并排坐 vs 一人一间房](docs/assets/zh-CN/collab-model.svg)

---

## 三、变现：调好几个，就导出几个包

花了好几天，才把客服、开发、测试这三个数字员工一起带顺——插件、预设、话术、谁盯谁，一点点调对。
...
**`.dshpack`：** 把已经调好的那一个数字员工（一整套 dsh）打成文件。客户导入即用；可绑机器码 / 密码，可禁止再导出——你卖的是成品，不是陪装服务。

调好几个数字员工，就导出几个包。一份 `.dshpack` 仍是一份 dsh；买家导入后出现在侧栏，分屏自己再拖。
